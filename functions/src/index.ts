...
import {
  ...
  sendTryOnCompleteNotification,
  sendBroadcastNotification,
  sendLocalizedBroadcast,
...
export { analyzeOutfitMinimal } from './minimalAnalysis';
...
async function verifyUser(uid: string): Promise<{
  ...
  const tryOnToday = isSameDay ? (userData.usage?.tryOnToday || 0) : 0;
...
async function incrementUsage(uid: string, type: 'analysis' | 'tryon'): Promise<void> {
  const userRef = db.collection('users').doc(uid);
...
const rateLimitCache = new Map<string, { count: number; resetTime: number; blocked: boolean; blockLevel: number }>();
const abuseDetectionCache = new Map<string, {
  failedAttempts: number;
  firstFailedAt: number;
  warningGiven: boolean;
  blockLevel: number; // 0: none, 1: warning, 2: temp ban, 3: longer ban
  lastBlockTime: number;
  legitimateErrors: number; // Network errors, timeouts etc.
}>();
...
function checkRateLimit(
  ...
  operationType: 'normal' | 'tryon' | 'polling' = 'normal' // Yeni parametre
...
async function isWhitelisted(uid: string): Promise<boolean> {
  try {
    ...
    return userData.flags?.isVIP === true || userData.flags?.isAdmin === true;
  } catch (error) {
...
function classifyError(errorCode: string, errorMessage: string): 'legitimate' | 'suspicious' | 'abuse' {
...
async function detectAbuse(
  identifier: string,
  errorType: 'legitimate' | 'suspicious' | 'abuse' = 'suspicious'
): Promise<{
  isAbuse: boolean;
  shouldBlock: boolean;
...
async function markUserForReview(uid: string, record: any): Promise<void> {
  try {
...
function validateRequest(data: any): { valid: boolean; reason?: string } {
  ...
  if (data.userImageBase64 || data.clothingImageBase64s) {
    const base64 = data.userImageBase64 as string;
  ...
  async (request) => {
    ...
    try {
      ...
      if (!validation.valid) {
        ...
        throw new HttpsError('invalid-argument', validation.reason || 'Geçersiz istek');
      }
      ...
      if (!ipRateLimitMinute.allowed) {
        ...
        throw new HttpsError('resource-exhausted', 'Sistem yoğun, lütfen birkaç saniye bekleyin.');
      }
      ...
      try {
        ...
        await sendAnalysisCompleteNotification(uid, score);
        ...
        if (error.code !== 'resource-exhausted') {
          ...
          if (errorType === 'suspicious' || errorType === 'abuse') {
            ...
            if (abuseResult.shouldBlock) {
              ...
              throw new HttpsError(
                'permission-denied',
                abuseResult.message || 'Your account has been temporarily restricted.'
              );
            }
          }
        }
...
function getPurposeDescriptions(purposes: string[], language: 'tr' | 'en'): string {
  const purposeMap: Record<string, { tr: string; en: string }> = {
    general: {
  ...
  const descriptions = purposes
    .map((p) => purposeMap[p]?.[language] || purposeMap.general[language])
    .join(', ');
...
Provide specific comments and recommendations based on these purposes.`;
  ...
  if (userPreferences) {
    if (language === 'tr') {
      ...
      if (userPreferences.usageGoals?.length) {
...
export const tryOutfit = onCall(
  ...
  async (request) => {
    ...
    console.log(`[tryOutfit][${requestId}] ===== NEW REQUEST =====`);
    console.log(`[tryOutfit][${requestId}] Timestamp: ${new Date().toISOString()}`);

    try {
      // 1. Authentication kontrolu
      if (!request.auth) {
        console.error(`[tryOutfit][${requestId}] Authentication failed: No auth token`);
        throw new HttpsError('unauthenticated', 'Giris yapmaniz gerekiyor');
      }

      uid = request.auth.uid;
      const clientIp = request.rawRequest.ip || 'unknown';
      console.log(`[tryOutfit][${requestId}] User ID: ${uid}, IP: ${clientIp}`);
      ...
      if (userStatus.isBlocked) {
        throw new HttpsError('permission-denied', 'Hesabiniz bloke edilmis');
      }
      ...
      // 5. Data validasyonu
      const {
        userImageBase64,        // Kullanicinin fotografinin base64'u
      ...
      console.log(`[tryOutfit][${requestId}] === REQUEST DATA DEBUG ===`);
      console.log(`[tryOutfit][${requestId}] Data keys:`, Object.keys(request.data));
      console.log(`[tryOutfit][${requestId}] userImageBase64 type:`, typeof userImageBase64);
      console.log(`[tryOutfit][${requestId}] userImageBase64 length:`, userImageBase64?.length || 0, `chars (${Math.round((userImageBase64?.length || 0) / 1024)}KB)`);
      console.log(`[tryOutfit][${requestId}] clothingImageBase64s type:`, typeof clothingImageBase64s);
      console.log(`[tryOutfit][${requestId}] clothingImageBase64s count:`, Array.isArray(clothingImageBase64s) ? clothingImageBase64s.length : 'not an array');
      console.log(`[tryOutfit][${requestId}] ========================`);

      if (!userImageBase64 || typeof userImageBase64 !== 'string') {
        console.error(`[tryOutfit][${requestId}] VALIDATION FAILED: userImageBase64 is missing or not a string`);
        throw new HttpsError('invalid-argument', 'Kullanici fotografi gerekli');
      }

      if (!clothingImageBase64s || !Array.isArray(clothingImageBase64s) || clothingImageBase64s.length === 0) {
        throw new HttpsError('invalid-argument', 'En az 1 kiyafet fotografi gerekli');
      }

      if (clothingImageBase64s.length > 10) {
        throw new HttpsError('invalid-argument', 'Maksimum 10 kiyafet fotografi yuklenebilir');
      }
      // 5. API key kontrolu
      ...
      if (!apiKey) {
        ...
        throw new HttpsError('internal', 'EachLabs API key yapilandirilmamis');
      }

      // 6. Base64 gorsellerini data URI formatina cevir
      const formatBase64ToDataUri = (base64: string): string => {
      ...
      console.log('EachLabs request body (summary):', {
        ...
        input: {
          ...requestBody.input,
          images: `[${requestBody.input.images.length} base64 images]`,
        },
      });
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          throw new HttpsError('internal', 'API anahtarı geçersiz veya süresi dolmuş. Lütfen daha sonra tekrar deneyin.');
        } else if (response.status === 429) {
          throw new HttpsError('resource-exhausted', 'Çok fazla istek. Lütfen birkaç dakika bekleyip tekrar deneyin.');
        } else if (response.status === 413) {
          throw new HttpsError('invalid-argument', 'Görseller çok büyük. Lütfen daha küçük görseller kullanın.');
        } else if (response.status === 400) {
          // Bad request - usually means invalid input
          throw new HttpsError('invalid-argument', `Geçersiz görsel veya parametreler. Lütfen farklı görseller deneyin. Detay: ${errorText.substring(0, 100)}`);
        } else if (response.status >= 500) {
          // Server error
          throw new HttpsError('unavailable', 'Görsel işleme servisi şu anda kullanılamıyor. Lütfen birkaç dakika sonra tekrar deneyin.');
        } else {
          throw new HttpsError('internal', `Görsel işleme servisi hatası (${response.status}). Lütfen tekrar deneyin.`);
        }
      }
      ...
      if (!predictionId) {
        ...
        console.error('=====================================');
        throw new HttpsError('internal', 'Prediction ID alinamadi');
      }
      ...
      if (error?.code === 'ECONNREFUSED' || error?.code === 'ETIMEDOUT' || error?.code === 'ENOTFOUND') {
        ...
        await detectAbuse(uid, 'legitimate');
        throw new HttpsError('unavailable', 'Görsel işleme servisine bağlanılamıyor. Lütfen internet bağlantınızı kontrol edin ve tekrar deneyin.');
      }
      ...
      if (error?.name === 'FetchError' || error?.message?.includes('fetch')) {
        ...
        await detectAbuse(uid, 'legitimate');
        throw new HttpsError('unavailable', 'API isteği başarısız oldu. Lütfen tekrar deneyin.');
      }

      // Bilinmeyen hatalar
      await detectAbuse(uid, 'legitimate');
      console.error(`[tryOutfit][${requestId}] Unknown error - throwing generic internal error`);
      console.error(`[tryOutfit][${requestId}] Error stack:`, error?.stack);

      throw new HttpsError('internal', `Kıyafet deneme sırasında beklenmeyen bir hata oluştu. Lütfen tekrar deneyin. (Request ID: ${requestId})`);
    }
  }
);

// Prediction durumu sorgulama fonksiyonu
export const getPredictionStatus = onCall(
  {
    region: 'europe-west1',
  ...
  async (request) => {
    ...
    if (!predictionId) {
      throw new HttpsError('invalid-argument', 'Prediction ID gerekli');
    }
    ...
    try {
      ...
      if (!predictionDoc.exists) {
        throw new HttpsError('not-found', 'Prediction bulunamadi');
      }

      const predictionData = predictionDoc.data()!;

      if (predictionData.userId !== uid) {
        throw new HttpsError('permission-denied', 'Bu prediction\'a erisim yetkiniz yok');
      }
      ...
      if (!response.ok) {
        ...
        throw new HttpsError('internal', 'Durum sorgulama hatasi');
      }
      ...
      const outputUrl = typeof result.output === 'string'
        ? result.output
        : (result.output?.images?.[0] || result.output?.[0] || result.output);
      ...
      if (status === 'success' || status === 'succeeded' || status === 'completed') {
        ...
        try {
          await sendTryOnCompleteNotification(uid, predictionId, outputUrl);
      ...
      throw new HttpsError('internal', 'Durum sorgulama sirasinda hata olustu');
    }
  }
);
...
const adminSecretKey = defineSecret('ADMIN_SECRET_KEY');

// Verify admin authentication
function verifyAdminAuth(providedKey: string, secretKey: string): boolean {
 ...
 * }
 */
export const sendLocalizedBroadcastNotification = onCall(
  {
    region: 'europe-west1',
    timeoutSeconds: 540,
    memory: '512MiB',
  ...
  async (request) => {
    ...
    if (!messages || typeof messages !== 'object') {
      throw new HttpsError('invalid-argument', 'Messages object is required');
    }

    if (!messages.tr || !messages.en) {
      throw new HttpsError('invalid-argument', 'Turkish and English messages are required');
    }
    ...
    try {
      ...
      const result = await sendLocalizedBroadcast(messages, data);
      ...
      throw new HttpsError('internal', 'Localized broadcast failed');
    }
  }
);
...
// ============================================
export { registerPushToken } from './pushTokenRegistration';
